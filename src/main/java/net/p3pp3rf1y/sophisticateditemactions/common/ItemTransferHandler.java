...
public class ItemTransferHandler {
	...
	public static void handleDeposit(Player player, int minSlot, int maxSlot, Map<ResourceLocation, List<BlockPos>> storagePositions, Map<ResourceLocation, List<Integer>> entities, boolean onlyMatching) {
		...
		ItemActionsPacketHandler.INSTANCE.sendToClient(serverPlayer, new SyncItemTransfersMessage(itemTransferData, playerPos, true));
		ItemActionsPacketHandler.INSTANCE.sendToAllTracking(new SyncItemTransfersMessage(itemTransferData, playerPos, true), serverPlayer);
		...
		if (maxSlot - minSlot == 1) {
			if (inserted.isEmpty()) {
				...
				message = ItemActionsTranslationHelper.INSTANCE.translStatusMessage("deposited_item",
						Component.literal(inserted.values().iterator().next().itemsTransferred().get(0).getHoverName().getString()).withStyle(ChatFormatting.DARK_GREEN));
			}
		} else {
			if (inserted.isEmpty()) {
	...
	private static void restockItem(Player player, ItemStack filter, int minSlot, int maxSlot, boolean fillEmpty, boolean refillSingle) {
		...
		if (!storages.isEmpty() || !entities.isEmpty()) {
			ItemActionsPacketHandler.INSTANCE.sendToServer(new RestockItemsMessage(filter, minSlot, maxSlot, fillEmpty, refillSingle, storages, entities));
		} else {
			playError(player, ItemActionsTranslationHelper.INSTANCE.translStatusMessage("no_storage_in_range").setStyle(Style.EMPTY.withColor(0xFF5555)));
		}
	}
	...
	public static void handleRestock(Player player, Map<ResourceLocation, List<BlockPos>> storagePositions, Map<ResourceLocation, List<Integer>> entities, int minSlot, int maxSlot, ItemStack filter, boolean fillEmpty, boolean refillSingle) {
		...
		ItemActionsPacketHandler.INSTANCE.sendToClient(serverPlayer, new SyncItemTransfersMessage(itemTransferData, playerPos, false));
		ItemActionsPacketHandler.INSTANCE.sendToAllTracking(new SyncItemTransfersMessage(itemTransferData, playerPos, false), serverPlayer);
		...
		if (maxSlot - minSlot == 1) {
			if (restocked.isEmpty()) {
				...
				message = ItemActionsTranslationHelper.INSTANCE.translStatusMessage("restocked_item",
						Component.literal(restocked.values().iterator().next().itemsTransferred().get(0).getHoverName().getString()).withStyle(ChatFormatting.DARK_GREEN));
			}
		} else {
			if (restocked.isEmpty()) {
